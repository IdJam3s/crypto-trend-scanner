...
SENDER_PASSWORD = "gexxdhkmrenfbzhf"  # Gmail App Password
...
# Binance futures (perpetual swaps)
BINANCE_FUTURES = ccxt.binance({
    'enableRateLimit': True,
    'options': {'defaultType': 'future'}
})

# Binance spot for fallback
...
include_spot_fallback = True  # True = assess spot if no futures data available
...
def run_scan():
    print(f"\n{'='*100}")
    print(f" JAMES' FULL-MARKET LONG-TREND SCANNER ({datetime.now():%b %d, %Y · %I:%M %p})")
    print(f"{'='*100}\n")

    futures_markets = BINANCE_FUTURES.load_markets()
    spot_markets = BINANCE_SPOT.load_markets() if include_spot_fallback else {}
    ...
    <body>
    <h1>James' Full-Market Long-Trend Scanner</h1>
    <p style="text-align:center;"><strong>Report Generated:</strong> {datetime.now():%B %d, %Y · %I:%M %p}</p>
    <p style="text-align:center;">Top 5 assets per timeframe/quote (Binance Perpetual Futures + Spot fallback).</p>
    """
    ...
    for quote_name, quote in QUOTES.items():
        ...
        # Fixed line: use 'swap' instead of 'perpetual'
        quote_symbols = [s for s in futures_markets if futures_markets[s]['active'] and futures_markets[s]['swap'] and s.endswith(f":{quote}")]

        for label, tf in TIMEFRAMES.items():
            rankings = []
            for symbol in quote_symbols:
                df = get_data(BINANCE_FUTURES, symbol, tf)
                if len(df) < 60:
                    if include_spot_fallback:
                        spot_symbol = symbol.replace(f":{quote}", "")
                        df = get_data(BINANCE_SPOT, spot_symbol, tf)
                        if len(df) < 60:
                            continue
                        symbol_type = "(Spot)"
                    else:
...
if __name__ == "__main__":
    html_body = run_scan()
    subject = f"Full-Market Long-Trend Scanner Report • {datetime.now():%b %d, %Y • %I:%M %p}"
